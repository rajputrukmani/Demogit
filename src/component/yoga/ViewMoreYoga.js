...
export default function ViewMoreYoga(){
    ...
    const getYogaById = async()=>{
     try{   
      ...
      console.log(response.data.yoga);
      setYoga(response.data.yoga);
     }
     catch(err){
        console.log(err);
     } 
      ...
      <div className="container mt-5">
        <div className="row d-flex justify-content-between">
            <div  className="col-md-6 d-flex flex-column" style={{height:"500px",boxShadow:"10px 10px 10px grey"}}>
              <img src={yoga.imageUrl} style={{width:"100%", height:"400px"}}/>  
              <div className="d-flex d-flex justify-content-center align-items-center">
                {yoga?.images?.map((imageUrl,index)=><img src={imageUrl} key={index} style={{width:"30%", height:"80px"}}/>)}
              </div>
            </div>
            <div className="col-md-6" style={{height:"500px", boxShadow:"10px 10px 10px grey"}}>
               <div className="p-3 d-flex flex-column"> 
                <h4>{yoga.yogaName} [{yoga.brand}] <del className="text-danger">{yoga.yogaName}</del></h4>
                { <p><b>{yoga?.benefits?.toUpperCase()}</b></p>}
